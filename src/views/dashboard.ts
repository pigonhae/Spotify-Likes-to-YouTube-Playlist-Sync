...
function clientScript() {
  return `
    ...
    async function refreshLive(showRecovered){ const res=await fetch('/api/dashboard/live',{headers:{accept:'application/json'}}); if(!res.ok) throw new Error('live refresh failed'); liveData=await res.json(); currentLanguage=liveData.language||currentLanguage; document.documentElement.lang=currentLanguage; replaceSection('header-root', liveData.sections?.header); replaceSection('overview-root', liveData.sections?.overview); replaceSection('live-root', liveData.sections?.live); replaceSection('recent-runs-root', liveData.sections?.recentRuns); replaceSection('attention-root', liveData.sections?.attention); replaceSection('danger-root', liveData.sections?.danger); trackState.runId=liveData.summary?.activeRun?.id??null; const filter=document.getElementById('track-filter'); if(filter instanceof HTMLSelectElement){ filter.value=trackState.filter; } if(trackState.runId && (trackState.filter!=="all" || trackState.page!==1)){ await loadTrackPage(); } setAlert(showRecovered?'success':'', showRecovered?tt('live.restoredBanner'):''); staleMode=false; pollDelay=liveData.summary?.activeRun?5000:20000; }
    ...
    async function switchLanguage(language){ if(language===currentLanguage) return; setAlert('success', tt('language.switching')); const res=await fetch('/api/preferences/language',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded',accept:'application/json'},body:new URLSearchParams({language}).toString()}); if(!res.ok){ setAlert('error', tt('live.loadingError')); return; } const payload=await res.json(); currentLanguage=payload.language||language; document.documentElement.lang=currentLanguage; await refreshLive(false); }
    ...
    document.addEventListener('visibilitychange',()=>{ if(document.visibilityState==='visible'){ void refreshLive(staleMode); } });
